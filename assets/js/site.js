  ...
  if (tabs && content) {
    ...
    const renderRules = (query='') => {
      ...
      filteredCategories.forEach(category => {
        const matchedItems = category.items.filter(([id, text]) => {
          const hay = `${category.title} ${category.intro} ${id} ${text}`.toLowerCase();
    ...
    if (search) {
      search.addEventListener('input', (event) => renderRules(event.target.value));
    }
  }
})();
