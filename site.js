(() => {
  ...
  async function updateLiveStats() {
    ...
    if (discordCard) {
      ...
      if (copy) {
        copy.textContent = 'Discord-Community live';
        copy.style.display = 'block';
      }
      discordCard.style.setProperty('--discord-bg', "url('assets/media/discord-panel-bg-dbg21.webp')");
    }
  ...
  badge.innerHTML = `<strong>Debug-Build ${DBG_VERSION}</strong><span>${pageMap[currentPath] || (document.body.dataset.page || 'Seite')}</span><span>Stylesheet: styles.css?v=dbg21</span><span>Discord-Panel sichtbar mit Bildhintergrund aktiv</span><span>FiveM: yg8z9k · Discord: YhVeud3Suz</span>`;
})();
